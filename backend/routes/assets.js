...
const { isValidDate, isValidPeriod, todayInJakarta } = require('../utils/date-validation')
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (acquisitionDate && acquisitionDate !== currentAcquisitionDate) {
      await db.query(
        `UPDATE journal_entries SET transaction_date = ? WHERE source_type = 'asset_acquisition' AND source_id = ? AND status = 'posted'`,
        [acquisitionDate, assetId],
      )
    }

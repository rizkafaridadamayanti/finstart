...
export function mapProject(row = {}) {
  return {
    ...
    status: projectStatus[String(row.status || '').toLowerCase()] || row.status || 'Planning',
...
export function mapAccount(row = {}) {
  return {
    ...
    tipe: accountTypes[String(row.type || '').toLowerCase()] || row.tipe || 'Aset',
...
export function mapJournalTransaction(row = {}) {
  ...
  return {
    ...
    debitAkun: accountCodeFromAllocation(debitAllocations[0]) || row.debitAkun || '',
    kreditAkun: accountCodeFromAllocation(creditAllocations[0]) || row.kreditAkun || '',
...
export function mapSubscription(row = {}) {
  ...
  return {
    ...
    siklus: cycle === 'yearly' || cycle === 'annual' || cycle === 'tahunan'
      ? 'Tahunan'
      : 'Bulanan',
    kategori: mapSubscriptionCategory(row.category || row.kategori),
    biaya: amount,
    biayaIDR: monthly,
    tanggalTagihan: dateOnly(row.renewal_date || row.next_billing_date || row.tanggalTagihan),
    status: row.status || '',
    _raw: row,
  }
}
...
export function mapAsset(row = {}) {
  ...
  return {
    ...
    nilaiBuku: bookValue,
    penanggungJawab: row.responsible_person || row.penanggungJawab || String(row.notes || '').replace(/^Penanggung jawab:\s*/i, '') || '-',
    status: row.status || '',
...
export function mapBill(row = {}) {
  ...
  return {
    ...
    nominal: numberValue(row.vendor_payable_amount ?? row.total_amount ?? row.nominal),
...
export function projectionTargets(projectionData = {}) {
  const summary = projectionData.summary || {}
  const year = projectionData.year || new Date().getFullYear()

  return [
    {
      id: `revenue-${year}`,
      nama: 'Target Pendapatan',
      nilaiTarget: numberValue(summary.revenue_target),
      nilaiRealisasi: numberValue(summary.revenue_actual),
      satuan: 'Rupiah',
    },
    {
      id: `expense-${year}`,
      nama: 'Target Beban Operasional',
      nilaiTarget: numberValue(summary.expense_target),
      nilaiRealisasi: numberValue(summary.expense_actual),
      satuan: 'Rupiah',
    },
    {
      id: `profit-${year}`,
      nama: 'Target Laba Bersih',
      nilaiTarget: numberValue(summary.profit_target),
      nilaiRealisasi: numberValue(summary.profit_actual),
      satuan: 'Rupiah',
    },
  ]
}
